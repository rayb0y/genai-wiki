...
The system prompt for each agent is in [[agent-prompts]]. A flow view of the whole system is in [[architecture-diagram]].
...
**Speaks to:** its assigned member (a private, one-to-one channel) and the Orchestrator.

The Collaboration Agent tracks whether each member is being heard, whether their stated positions and objections are being addressed in subsequent sessions, and whether participation patterns are becoming unbalanced. It uses the session transcripts and the member's structured post-session notes as its primary inputs. The research base for what the system should do when a member brings a complaint, about a teammate or about the project, including the triangulation risk that a third party which absorbs the tension lets people avoid the accountable conversation, is in [[complaints-and-conflict-in-teams]].
...
The member-facing channel: each member's private link to the system is their own Collaboration Agent. It is where they file post-session notes and where they raise concerns or complaints between sessions, one-to-one and privately. This is the second team-facing channel in the system, and it is different in kind from the Socratic Agent's. The Socratic Agent addresses the whole team with one shared question; the Collaboration Agent only ever interacts with its single assigned member, and what the member tells it stays in that private channel. Nothing a member says to their agent is surfaced to the team verbatim; if it warrants a team-level response, the Orchestrator decides that and the response goes out as a neutral, de-identified Socratic question, never as "someone complained." That confidentiality is what makes the channel usable, because a member who fears their words will be repeated to the group stops being honest with their agent.
...
What it can and cannot attribute about who built what, why a finished artifact with everyone's name on it usually cannot be traced to one author, and how to record contribution as a confirmable provenance map rather than a covert score, is researched in [[proof-of-work-attribution]].
...
**Source 3 (Socratic answers):** When the Socratic Agent asks a rationale-probing question, the question is logged to the Tier-4 question-and-answer history so a later answer can be paired with it. The answer does not come to the Decision Logger directly from the Socratic Agent. It re-enters on the same input path as every other team material (the feedback channel), gets written to that Tier-4 history against its question, and is parsed for the decision and reason it contains, the same extraction every transcript gets. The Decision Logger reads it from there and records it as a decision log entry, whether or not the team ever wrote one formally. A logged "ignoring this question because we addressed it last session" is recorded too, documenting what the team considered and rejected. This is why the feedback loop's "ignoring" option requires a written reason rather than a silent dismiss. Routing the answer back through ingestion rather than handing it straight to the Decision Logger keeps one input path for all team material and makes the answer visible to the other detectors too, not just this one.
...
**Writes:** nothing to Project State. Its question is logged to the Tier-4 question-and-answer history when asked, so the team's answer can later be paired with it.
**Speaks to:** the whole team (the only agent that addresses the team collectively, through the Orchestrator's trigger; the Collaboration Agent speaks only to its one member, privately).
...
Why the Socratic Agent writes nothing to Project State: a question that does not land is not a decision. If the Socratic Agent wrote every question it fired into the decision log, that log would accumulate questions the team found irrelevant and the signal-to-noise ratio for the Decision Logger would degrade. The question is kept in the Tier-4 question-and-answer history instead, not the decision log. The team's answer is not handed from the Socratic Agent to the Decision Logger; it re-enters through the same input path as every other team material, is logged to that Tier-4 history, and is read from there by the Orchestrator as a calibration signal and by the Decision Logger as a substantive entry.
